DigitalSignage.grid.Slides = function(config) {
    ...
    config.tbar = [{
        ...
        text        : _('digitalsignage.slides_bulk_remove'),
        handler     : this.bulkRemoveSlides,
        scope       : this
    }, '-', {
        text        : _('digitalsignage.slides_bulk_depublish'),
        handler     : this.bulkDepublishSlides,
        scope       : this
    }, '->', {
...
Ext.extend(DigitalSignage.grid.Slides, MODx.grid.Grid, {
    ...
    bulkRemoveSlides: function(btn, e) {
        var selections = this.getSelectionModel().getSelections();

        if (selections.length === 0) {
            MODx.msg.alert(_('error'), _('digitalsignage.slides_bulk_remove_error_empty'));
            return;
        }
        ...
        MODx.msg.confirm({
            title       : _('digitalsignage.slides_bulk_remove'),
            text        : _('digitalsignage.slides_bulk_remove_confirm', {count: ids.length}),
            url         : DigitalSignage.config.connector_url,
            params      : {
                action      : 'mgr/slides/bulkremove',
                ids         : ids.join(',')
            },
            listeners   : {
                'success'   : {
    ...
    bulkDepublishSlides: function(btn, e) {
        var selections = this.getSelectionModel().getSelections();

        if (selections.length === 0) {
            MODx.msg.alert(_('error'), _('digitalsignage.slides_bulk_depublish_error_empty'));
            return;
        }
        ...
        MODx.msg.confirm({
            title       : _('digitalsignage.slides_bulk_depublish'),
            text        : _('digitalsignage.slides_bulk_depublish_confirm', {count: ids.length}),
            url         : DigitalSignage.config.connector_url,
            params      : {
                action      : 'mgr/slides/bulkdepublish',
                ids         : ids.join(',')
            },
            listeners   : {
                'success'   : {
                ...
                Ext.iterate(record.data.data, function(name, record) {
                    ...
                    switch (record.xtype) {
                        ...
                        case 'modx-combo-browser':
                            // Set size based on slide type: default = 1536x1024, media (and other types) = 1080x1920
                            var cropSize = (type === 'default') ? '1536x1024' : '1080x1920';
                            
                            record = Ext.apply(record, {
                                xtype   : 'imagecropper-combo-browser',
                                sizes   : {
                                    crop   : {
                                        name    : _('digitalsignage.slide'),
                                        size    : cropSize
                                    }
                                },
                ...
                Ext.iterate(record.data.data, function(name, record) {
                    ...
                    switch (record.xtype) {
                        ...
                        case 'modx-combo-browser':
                            ...
                            // Get existing value and check if crop data needs to be cleared due to size change
                            var existingValue = this.config.record.data[name] || '';
                            var valueToSet = existingValue;
                            
                            if (existingValue && typeof existingValue === 'string' && existingValue.indexOf('{') === 0) {
                                try {
                                    var parsedValue = Ext.decode(existingValue);
                                    if (parsedValue.sizes && parsedValue.sizes.crop) {
                                        // Check if the existing crop size matches the new size
                                        var existingSize = parsedValue.sizes.crop.size || '';
                                        if (existingSize !== cropSize) {
                                            // Size changed - clear crop data but keep the image
                                            valueToSet = Ext.encode({
                            ...
                            // Store the value to set after component is created
                            record._valueToSet = valueToSet;
                    ...
                    setTimeout(function(){
                        // Use custom value if set (for image cropper with size change), otherwise use existing value
                        var fieldValue = (record._valueToSet !== undefined) ? record._valueToSet : (wrapper.config.record.data[name] || '');
                        delete record._valueToSet; // Clean up temporary property
