...
Ext.ensible.cal.WeekEventRenderer = function() {
     ...
     */
    var getEventRow = function(viewId, weekIndex, rowIndex) {
     ...
     * @private
     */
    var renderEvent = function(event, weekIndex, dayIndex, eventIndex, dayCount, currentDate, renderConfig) {
    ...
    return {
        ...
        render: function(config) {
            ...
            var me = this,
                ...
                currentDate = config.viewStart.clone(),
                currentDateString = '',
                eventTpl = config.tpl,
                    ...
                    if (weekGrid && weekGrid[dayIndex]) {
                        ...
                        for(; eventIndex < eventCount; eventIndex++){
                            if (!dayGrid[eventIndex]) {
                                ...
                                eventRow = getEventRow(config.viewId, weekIndex, eventIndex);
                                ...
                                if (!currentEvent.isSpan || currentEvent.isSpanStart) {
                                    renderEvent(currentEvent, weekIndex, dayIndex, eventIndex,
                        ...
                        if (skippedEventCount > 0) {
                            ...
                            eventRow = getEventRow(config.viewId, weekIndex, maxEventsPerDay);
                            Ext.DomHelper.append(eventRow, {
                                tag: 'td',
                                cls: 'ext-cal-ev-more',
                                id: 'ext-cal-ev-more-' + currentDate.format('Ymd'),
                                cn: {
                                    tag: 'a',
                                    html: String.format(config.getMoreText(skippedEventCount), skippedEventCount)
                                }
                            });
                        }
                            ...
                            eventRow = getEventRow(config.viewId, weekIndex, eventCount);
                            if (eventRow) {
                                cellConfig = {
                                    tag: 'td',
                                    cls: 'ext-cal-ev',
                        ...
                        eventRow = getEventRow(config.viewId, weekIndex, 0);
                        if (eventRow) {
                            cellConfig = {
                                tag: 'td',
                                cls: 'ext-cal-ev',
        ...
        evts.each(function(evt) {
            ...
            if (Ext.ensible.Date.diffDays(evt.data[M.StartDate.name], evt.data[M.EndDate.name]) > 0) {
                var daysInView = Ext.ensible.Date.diffDays(
                    Ext.ensible.Date.max(me.viewStart, evt.data[M.StartDate.name]),
                    Ext.ensible.Date.min(me.viewEnd, evt.data[M.EndDate.name])) + 1;
