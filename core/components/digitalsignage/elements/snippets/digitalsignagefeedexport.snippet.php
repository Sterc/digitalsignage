            ...
            if (isset($info['http_code']) || '200' == $info['http_code']) {
                if (isset($info['content_type'])) {
                    if (strstr($info['content_type'], 'xml')) {
                        ...
                        if (false !== ($response = simplexml_load_string($response))) {
                            if (isset($response->channel->item)) {
                                foreach ($response->channel->item as $value) {
                                    ...
                                    $data[$key][] = array(
                                        ...
                                        'image'         => $image,
                                        'content'       => trim(preg_replace('/\s+/', ' ',str_replace('&nbsp;', '', htmlentities(html_entity_decode(strip_tags((string) (isset($value->description) ? $value->description : ''))))))),
