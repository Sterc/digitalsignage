<?php
	...
	class NarrowcastingSlideTypesDataCreateProcessor extends modProcessor {
		/**
		 * @access public.
		 * @var String.
		 */
            ...
            if (null !== ($object = $this->modx->getObject($this->classKey, $this->getProperty('id')))) {
                if (!preg_match('/^([a-zA-Z0-9\_\-]+)$/si', $this->getProperty('key'))) {
                    ...
                    if (isset($data[$this->getProperty('key')])) {
                        $this->addFieldError('key', $this->modx->lexicon('narrowcasting.error_slide_type_date_exists'));
                    } else {
	...
	return 'NarrowcastingSlideTypesDataCreateProcessor';
