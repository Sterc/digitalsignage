DigitalSignage.grid.Players = function(config) {
	...
	config.tbar = [{
		...
		xtype		: 'checkbox',
		name		: 'digitalsignage-refresh-players',
        id			: 'digitalsignage-refresh-players',
        ...
        xtype		: 'textfield',
        name 		: 'digitalsignage-filter-search-players',
        id			: 'digitalsignage-filter-search-players',
        emptyText	: _('search')+'...',
        listeners	: {
	        'change'	: {
    	...
    	id			: 'digitalsignage-filter-clear-players',
    	text		: _('filter_clear'),
    	listeners	: {
        	'click'		: {
        		fn			: this.clearFilter,
...
Ext.extend(DigitalSignage.grid.Players, MODx.grid.Grid, {
    ...
    clearFilter: function() {
	    ...
	    Ext.getCmp('digitalsignage-filter-search-players').reset();
	    
        this.getBottomToolbar().changePage(1);
    },
    getMenu: function() {
        return [{
	    	text 	: _('digitalsignage.player_view'),
	    	handler : this.viewPlayer,
	    	scope 	: this 
	    }, '-', {
            text	: '<i class="icon icon-power-off"></i> ' + (0 == this.menu.record.restart ? _('digitalsignage.player_restart') : _('digitalsignage.player_restart_cancel')),
            handler	: this.restartPlayer,
            scope	: this
        }, '-', {
	        text	: _('digitalsignage.player_update'),
	        handler	: this.updatePlayer,
	        scope	: this
	    }, {
	        text	: _('digitalsignage.player_schedule'),
	        handler	: this.schedulePlayer,
	        scope	: this
	    }, '-', {
		    text	: _('digitalsignage.player_remove'),
    ...
    restartPlayer: function() {
        MODx.msg.confirm({
            title 		: 0 == this.menu.record.restart ? _('digitalsignage.player_restart') : _('digitalsignage.player_restart_cancel'),
            text		: 0 == this.menu.record.restart ? _('digitalsignage.player_restart_confirm') : _('digitalsignage.player_restart_cancel_confirm'),
            url			: DigitalSignage.config.connector_url,
            params		: {
...
DigitalSignage.window.SchedulePlayer = function(config) {
    ...
    Ext.applyIf(config, {
        ...
        buttons 	: [{
	        text		: _('digitalsignage.player_calendar'),
